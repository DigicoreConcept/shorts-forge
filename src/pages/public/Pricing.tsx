...
const faqs = [
  ...
  {
    q: "What happens if I exceed my monthly upload limit?",
    a: "If you exceed your limit, you can either upgrade to the next tier or wait until your billing cycle resets. We also offer pay-as-you-go top-ups for one-off needs.",
  },
  {
    q: "Do you offer refunds?",
    a: "We offer a 14-day money-back guarantee for all new subscriptions. If you're not satisfied, just let us know within the first 14 days and we'll refund your payment.",
  },
  {
    q: "Can I switch plans later?",
    a: "Absolutely. You can upgrade or downgrade your plan at any time. Prorated charges or credits will be applied to your account automatically.",
  },
];
    ...
    <div className="flex flex-col w-full bg-[#FFEBEE]">
      ...
      <section className="pb-24 px-4 border-b border-[#FFCDD2]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="relative group">
            ...
            {/* Blurred Content */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center mb-16 blur-md select-none opacity-40">
              {plans.map((plan, i) => (
                <motion.div
                  key={plan.name}
                  initial={{ opacity: 0, y: 24 }}
                ...
                >
                  ...
                  <h3 className="font-bold text-2xl text-[#1A1A1A] mb-2 text-center">
                    {plan.name}
                  </h3>
                  <p className="text-sm text-[#616161] text-center mb-6">
                    {plan.desc}
                  </p>
      ...
      <section className="py-24 px-4 bg-[#FFF5F5] border-b border-[#FFCDD2]">
        <div className="max-w-6xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-16">
            <h2 className="text-4xl font-bebas text-[#1A1A1A]">
              Compare Features
            </h2>
          </motion.div>
          ...
          <div className="overflow-x-auto relative">
            <div className="absolute inset-0 z-50 flex items-center justify-center">
              <span className="px-6 py-3 bg-[#1A1A1A] text-white font-bebas text-2xl rounded shadow-2xl border border-[#333]">
                ALL FEATURES UNLOCKED FOR BETA USERS
              </span>
            </div>
            ...
            <table className="w-full text-left border-collapse min-w-[1000px] blur-md pointer-events-none select-none opacity-40">
              <thead>
                <tr>
                  ...
                  <th className="py-6 px-6 font-bold text-[#1A1A1A] text-lg w-[20%] text-center">
                    Free
                  </th>
                  <th className="py-6 px-6 font-bold text-[#1A1A1A] text-lg w-[20%] text-center">
                    Starter
                  </th>
                  <th className="py-6 px-6 font-bold text-[#EF5350] text-lg w-[20%] text-center border-b-2 border-[#EF5350]">
                    Creator
                  </th>
                  <th className="py-6 px-6 font-bold text-[#1A1A1A] text-lg w-[20%] text-center">
                    Agency
              ...
              <tbody>
                {comparisonFeatures.map((f, i) => (
                  <tr
                    key={f.name}
                    className={`${i % 2 === 0 ? "bg-[#FFFFFF]" : "bg-[#FFF5F5]"} border-b border-[#FFCDD2]`}
                  >
                    <td className="py-5 px-6 font-medium text-[#1A1A1A]">
                      {f.name}
                    </td>
                    ...
                    <td className="py-5 px-6 text-center text-[#616161]">
                      {typeof f.free === "boolean" ? (
                        f.free ? (
                          <Check size={20} className="mx-auto text-[#EF5350]" />
                        ) : (
                          <Minus size={20} className="mx-auto text-[#FFCDD2]" />
                        )
                      ) : (
                        f.free
                      )}
      ...
      <section className="py-24 px-4 bg-[#FFEBEE]">
        <div className="max-w-3xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-16">
            <h2 className="text-4xl font-bebas text-[#1A1A1A] mb-4">
              Pricing FAQs
            </h2>
          </motion.div>
